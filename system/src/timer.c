 ...
 */
static uint8_t TimerExists( TimerEvent_t *obj );
...
void TimerStart( TimerEvent_t *obj )
{
    ...
    if( ( obj == NULL ) || ( TimerExists( obj ) == SET ) )
    {
        BoardEnableIrq( );
        return;
    }
    ...
    {
        if( TimerListHead->IsRunning == SET )
        {
...
void TimerStop( TimerEvent_t *obj )
{
    ...
    {
        if( TimerListHead->IsRunning == SET ) // The head is already running
        {
            elapsedTime = TimerGetValue( );
...
{
    if( ( TimerListHead != NULL ) && ( TimerListHead->IsRunning == SET ) )
    {
